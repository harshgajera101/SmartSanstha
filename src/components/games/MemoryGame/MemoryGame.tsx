...
import { Brain, RotateCcw, Trophy, Target, CheckCircle, Star, Clock } from 'lucide-react';
    ...
    <div className="w-full max-w-7xl animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 bg-slate-800/80 backdrop-blur-sm rounded-xl px-4 py-3 shadow-lg border border-slate-700">
          ...
          <span className="text-sm font-semibold text-slate-300">Time: {formatTime(timeElapsed)}</span>
        </div>
        <div className="flex items-center gap-2 bg-slate-800/80 backdrop-blur-sm rounded-xl px-4 py-3 shadow-lg border border-slate-700">
      ...
      <main className={`grid gap-4 sm:gap-6 ${gridCols} mb-8`}>
        {cards.map((c, idx) => (
          <button 
            key={c.uid} 
            className={`group perspective w-full rounded-xl h-36 sm:h-44 md:h-48 lg:h-52 focus:outline-none focus:ring-4 focus:ring-orange-500/50 transition-all duration-300 ${c.matched ? 'opacity-50' : ''}`} 
          ...
          >
            <div className={`relative w-full h-full transition-transform duration-700 transform-style-preserve-3d ${c.flipped || c.matched ? 'rotate-y-180' : ''} group-hover:scale-105`}>
              <div className="absolute inset-0 backface-hidden rounded-xl shadow-lg bg-gradient-to-br from-slate-700 to-slate-800 p-6 flex flex-col items-center justify-center text-white border border-slate-600">
                <div className="relative z-10 text-center">
                  ...
                  <div className="text-lg sm:text-xl font-bold tracking-wider mb-2">SmartSanstha</div>
                  <div className="text-xs opacity-70 text-slate-400">Tap to Flip</div>
                </div>
              </div>
              <div className="absolute inset-0 rotate-y-180 backface-hidden rounded-xl shadow-xl bg-white text-slate-800 border-2 border-orange-500 p-4 sm:p-6 flex flex-col justify-between">
                <div className="flex items-start justify-between">
                  <div className="text-xs text-orange-700 font-semibold bg-orange-100 px-2 py-1 rounded-full">{c.matched ? 'MATCHED' : 'ARTICLE'}</div>
                  {c.matched && (
                ...
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-2">{c.article}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="text-center">
            ...
            <div className="bg-slate-700 border border-slate-600 rounded-xl p-6 mb-6 space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-orange-400">{best || moves} moves</span>
              </div>
            </div>
